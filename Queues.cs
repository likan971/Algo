...
{
    ///<summart>
    /// LeetCode 621 (best solution: priority queue/heap)
    ///</summary>
    public int LeastInterval(char[] tasks, int n)
    {
        ...
        dic = dic.OrderByDescending(item => item.Value).ToDictionary(item => item.Key, item => item.Value);

        var interval = n + 1;
        var i = 0;
        var count = 0;
        while(dic.Values.Any(v => v > 0))
        {
            i = 0;
            ...
            dic = dic.OrderByDescending(item => item.Value).ToDictionary(item => item.Key, item => item.Value);
            if (i < interval && dic.Values.Any(v => v > 0))
            {
                count += (interval - i);
            }
        }
